...
export class TopbarComponent {
	...
	protected readonly toggleLabel = computed(() => {
		...
		return this.mode() === 'dark'
			? this._translateService.translate('Μετάβαση σε φωτεινή λειτουργία')()
			: this._translateService.translate('Μετάβαση σε σκοτεινή λειτουργία')();
	});
	protected readonly languageMenuLabel = computed(() => {
		this.activeLanguage();
		return this._translateService.translate('Άνοιγμα μενού γλωσσών')();
	});
	protected readonly languageCycleLabel = computed(
		() => {
			this.activeLanguage();
			return `${this._translateService.translate('Αλλαγή γλώσσας σε')()} ${this.getNextLanguage().nativeName}`;
		},
	);
