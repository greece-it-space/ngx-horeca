...
function _buildFacts(item: Dish, category: DishCategory | null): DishFact[] {
	return [
		...
		},
		{
			...
			fallbackValue: item.portion ? undefined : 'Ρωτήστε το προσωπικό για λεπτομέρειες μερίδας',
		},
		{
			label: 'Χρόνος παρασκευής',
			value: item.cookTimeMinutes === null ? undefined : `${item.cookTimeMinutes} min`,
			fallbackValue: item.cookTimeMinutes === null ? 'Ρωτήστε το προσωπικό του εστιατορίου' : undefined,
		},
		{
			label: 'Θερμίδες',
			value: item.caloriesKcal === null ? undefined : `${item.caloriesKcal} kcal`,
			fallbackValue: item.caloriesKcal === null ? 'Ρωτήστε το προσωπικό του εστιατορίου' : undefined,
		},
		{
			label: 'Αλλεργιογόνα',
			translatedValue: item.allergens.length > 0 ? item.allergens.join(', ') : undefined,
			fallbackValue:
				item.allergens.length > 0 ? undefined : 'Δεν υπάρχουν πληροφορίες αλλεργιογόνων',
		},
	];
}
