...
import { findFallbackJobBySlug } from '../../feature/bootstrap/fallback-data';
import { companyProfile } from '../../feature/company/company.data';
import type { Job } from '@wawjs/ngx-horeca';
...
export class JobComponent {
	...
	protected readonly infoFields = computed<JobInfoField[]>(() => {
		...
		return [
			{ label: 'Τμήμα', value: job.department },
			{ label: 'Επίπεδο', value: job.level },
			{ label: 'Μορφή εργασίας', value: job.workFormat },
			{ label: 'Τοποθεσία', value: job.location },
			{ label: 'Τύπος απασχόλησης', value: job.employmentType },
			{ label: 'Πρόγραμμα', value: job.schedule },
			{ label: 'Μισθός', value: job.salary },
			{ label: 'Εμπειρία', value: job.experience },
