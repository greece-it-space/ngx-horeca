...
type BookingKind = 'τραπέζι' | 'δωμάτιο' | 'Σπα';
...
export class BookComponent {
	...
	protected readonly bookingOptions: BookingOption[] = [
		{
			...
			icon: 'restaurant',
			route: '/menu',
			description: 'Κλείστε τραπέζι και δείτε τα πιάτα πριν την επίσκεψή σας.',
		},
		{
			value: 'δωμάτιο',
			label: 'Δωμάτιο',
			icon: 'hotel',
			route: '/rooms',
			description: 'Ρωτήστε για διαθέσιμα δωμάτια και υπηρεσίες επισκεπτών.',
		},
		{
			value: 'Σπα',
			label: 'Σπα',
			icon: 'SPA',
			route: '/spa',
			description: 'Προγραμματίστε επίσκεψη wellness, μασάζ, σάουνα ή χρόνο αποκατάστασης.',
		},
	];
	...
	protected readonly bookingForm = form(
		this.bookingRequest,
		(path) => {
			required(path.phone, { message: 'Απαιτείται αριθμός τηλεφώνου' });
		},
		{
			name: 'booking',
			submission: {
				action: async () => {
					...
					try {
						...
						this.submitMessage.set('Το αίτημα αποθηκεύτηκε');
					} catch {
						this.submitError.set(
							'Δεν ήταν δυνατή η αποστολή του αιτήματος. Δοκιμάστε ξανά ή καλέστε μας.',
						);
					}
